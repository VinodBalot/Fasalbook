...
public class LocationProvider {
     ...
     */
    private static final int LOCATION_UPDATE_MIN_TIME_GPS = 1000; // default value is 1000.

    /**
     * location updates should fire, even if last signal is same than current one (0m distance to last location is OK)
     */
    private static final int LOCATION_UPDATE_DISTANCE_GPS = 2; // default value is 0.

    /**
     * location updates should fire approximately every second
     */
    private static final int LOCATION_UPDATE_MIN_TIME_NW = 1000; // default value is 1000.

    /**
     * location updates should fire, even if last signal is same than current one (0m distance to last location is OK)
     */
    private static final int LOCATION_UPDATE_DISTANCE_NW = 2; // default value is 0.

    /**
     * to faster access location, even use 10 minute old locations on start-up
     */
    private static final int LOCATION_OUTDATED_WHEN_OLDER_MS = 1000 * 60 * 10; // default value is 1000 * 60 * 10.
    ...
    public LocationProvider(@NonNull final Context context, @NonNull final LocationListener locationListener, @NonNull final ErrorCallback callback) {
        super();
