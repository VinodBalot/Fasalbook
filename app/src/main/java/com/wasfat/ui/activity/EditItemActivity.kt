...
class EditItemActivity : BaseBindingActivity() {
    ...
    private fun showImageSelectionDialog() {
        ...
        builder.setItems(options) { dialog, item ->
            if (options[item] == "Take Photo") {
                EasyImage.openCameraForImage(this@EditItemActivity, 100)
            } else if (options[item] == "Choose from Gallery") {
                EasyImage.openGallery(this@EditItemActivity, 200)
            ...
            object : DefaultCallback() {
                ...
                override fun onCanceled(source: EasyImage.ImageSource, type: Int) {
                    if (source == ImageProvider.CAMERA) {
                        val photoFile =
                            EasyImage.lastlyTakenButCanceledPhoto(this@EditItemActivity)
                        photoFile?.delete()
                    }
                }
            })
